...
class Access:
    ...
    >>> access.enter_system(User(5, 5, "Admin"), 5, "Степан")
    Traceback (most recent call last):
    seminar_13.custom_errors.LevelError: В доступе отказано. Ваш уровень ниже 5
    >>> access.enter_system(User(3, 5, "Admin"), 3, "User")
    Traceback (most recent call last):
...
def test_enter_system_level_error(make_access):
    with pytest.raises(LevelError):
        make_access.enter_system(User(5, 5, "Admin"), 5, "Степан")


def test_enter_system_access_error(make_access):
    with pytest.raises(AccessError):
        make_access.enter_system(User(3, 5, "Admin"), 3, "User")
...
class TestAccess(unittest.TestCase):
    ...
    def test_level_error(self):
        with self.assertRaises(LevelError):
            self.access.enter_system(User(5, 5, "Admin"), 5, "Степан")

    def test_access_error(self):
        with self.assertRaises(AccessError):
            self.access.enter_system(User(3, 5, "Admin"), 3, "User")
